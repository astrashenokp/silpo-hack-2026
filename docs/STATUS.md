...
| Polina | **Live, deployed and frozen at `main` @ `6784526`.** QA runs 1–17 complete: full toolkit sweep stable 3 times, live FatSecret credentials, live Silpo MCP with a real account, a Docker rehearsal, a public Northflank deployment (https://p01--web--2n7f5yvrbnqy.code.run), three browser-agent batteries, chat genuinely working with a real Gemini key, and a same-day production outage (Edamam 403) found, fixed and verified live — e2e 40/40 and UI 56/56 against that exact URL; September 14 | [QA results](qa/test-results.md), [bugs](qa/bugs.md), [demo script](qa/demo-script.md), [submission checklist](qa/submission.md), [handoff draft](handoffs/polina.md), [code review](qa/code-review.md) | Demo freeze and recording | No blocker or High defect open in Polina-tested paths on the live deploy. Remaining Low/Medium: BUG-007–BUG-010, BUG-013, BUG-017, BUG-021, BUG-024 (Ksiusha/Katia/Rina/Uliana); BUG-026 (live Edamam meals leave the basket empty — Sofiia/Rina, deliberately not enabled on the live deploy); CR-04 recurring-purchase matching still open (Rina/Vika/Uliana). Blocked on the team: a dedicated demo Silpo account or an agreed reset step (the connected account is a teammate's real one), presenter and submission-account owner. Registration confirmed by the team on Sep 13 |
