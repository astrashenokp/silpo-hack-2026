...
| BUG-036 | High | Live FatSecret Saved Meal export for a connected account always ended `partial`/`failed` with `UPSTREAM_UNAVAILABLE` (surfaced by BUG-035's mislabeled text as a Silpo outage): the meal itself saved (visible in the FatSecret panel), but item write-verification failed every time. Root cause: `_matches()` in `fatsecret/export.py` compared `food_id`/`serving_id`/`number_of_units` straight off the FatSecret response, but FatSecret wraps these as `{"value": ...}` on `saved_meal_item.add`/`saved_meal_items.get.v2` (the same quirk `_value()` already unwraps for `saved_meal_id` from `saved_meal.create`) — so every read-back comparison silently failed against the real API while always passing against the test fake, which echoed back unwrapped scalars | Polina, in Rina's `fatsecret/export.py` (user-authorised, third explicit override tonight after BUG-031 and BUG-034) | Fixed: `_matches()` now unwraps via the same `_scalar()`/`_value()` pattern. New regression test `test_live_export_verifies_writes_when_fatsecret_wraps_item_scalar_fields` reproduces the wrapped-field shape and fails without the fix (confirmed: `status: partial` before, `success` after). Full suite 299/299 ✅ |
